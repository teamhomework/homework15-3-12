...
namespace personremainer
{
    class DataBase
    {
        ...
        //向用戶操作表增加數據
        public void AddUserOp(string TableName,OptrecordNode UserOp_hand)
        {
            ...
            {
                string insertsql = "INSERT INTO " + "\""+ TableName + "\""+" (name,id, date, type ,price ,quantity ,taxrate ,commission)" + "VALUES ( " + "'" + UserOp.stockname + "','" + UserOp.stockcode + "','" + UserOp.optdate + "','" + UserOp.opttype + "','" + UserOp.stockprice + "','" + UserOp.stocknumber + "','" + UserOp.rate + "','" + UserOp.commission + "'" + ")";
        ...
        public DataSet ReadDB(string tablename, string search,int i)
        {
            ...
            try
            {
                ...
                if (search != "*")
                {
                    ...
                    if (1 == i)
                    {
                        searchsql = searchsql + " ORDER BY date ";
                    }
                }
                else
                {
